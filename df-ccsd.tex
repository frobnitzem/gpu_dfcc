...
% lite residual (Uvo Uvo computed in UpdateT2)
R^{ab}_{ij} &= A^{ab}_{ij} + B^{ab}_{ij}
...
%
%"u2"^{pq}_{rs} &= 2 t^{pq}_{rs} - t^{pq}_{sr} \\
% Singles residual
"w1"^a_i &= "Fai"_{ai} + "A1"^a_i + "B1"^a_i + "C1"^a_i \\
%"w1"^a_i &= "Fai"_{ai} + "C1"^a_i \\
% A1 / B1 check out, but somehow change the algo. result
